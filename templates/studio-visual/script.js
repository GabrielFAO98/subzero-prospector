/**
 * STUDIO VISUAL TEMPLATE - ORQUESTRACAO DE ANIMACOES & MIDIA
 * Direcao de Arte: Transicoes fluidas, cadencia cinematografica e luxo silencioso
 * Ideal para: Empresas de Imagem, Estetica Avancada, Fotografia Autoral, Artistas Visuais & Arquitetura
 */
...
function initHeroExperience() {
  ...
  // Auto-split do nome em caracteres individuais caso substituido por texto simples
  const nameRow = document.getElementById('nameRow');
  if (nameRow && nameRow.children.length === 0 && nameRow.textContent.trim()) {
    const rawText = nameRow.textContent.trim();
    nameRow.innerHTML = '';
  ...
  if (video) {
    ...
    video.addEventListener('error', (e) => {
      console.warn('[Studio Visual Hero] Erro no vídeo do header', e);
      triggerHeroEnter();
    });
  }
...
/**
 * Spotlight Dinamico de Luz na Foto Editorial
 */
